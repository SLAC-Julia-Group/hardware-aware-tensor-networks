...
class CascadedModel(Model):
    ...
    def load(cls, filepath: str, configure_training: bool = False, 
        ...
        from ..builders.encoder import EncoderBuilder

        builder = EncoderBuilder(debug=False)
        cascade = builder.create_encoder(
...
def create_trainable_encoder(layer_dims: List[int],
                               cyclic: bool = False,
    ...
    # Create cascade
    builder = EncoderBuilder(debug=debug)
    cascade = builder.create_encoder(
        layer_dims=layer_dims,
        cyclic=cyclic,
        key=key,
