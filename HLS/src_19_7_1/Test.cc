...
void load_input(
    ...
    data_t encoded_input[LAYER1_INPUT_SITES][LAYER1_PHYS_IN]
) {
    int feature_idx = 0;
...
void scale_input(
    ...
    for (int i = 0; i < LAYER1_INPUT_SITES; i++) {
        for (int j = 0; j < LAYER1_PHYS_IN; j++) {
...
void compute_initial_norm(
    const data_t encoded_input[LAYER1_INPUT_SITES][LAYER1_PHYS_IN],
    data_t* initial_norm_squared
) {
    *initial_norm_squared = 1.0;

    for (int site = 0; site < LAYER1_INPUT_SITES; site++) {
        data_t site_norm_sq = 0.0;
        for (int phys = 0; phys < LAYER1_PHYS_IN; phys++) {
...
int main(int argc, char **argv) {
   ...
   std::cout << "Layer 1, Site 18, phys_in=0, phys_out=1:" << std::endl;
   std::cout << "  weights1_data[18][0][1][0][0] = " << weights1_data[18][0][1][0][0] << std::endl;
   std::cout << "Layer 2, Site 0, phys_in=0, phys_out=0:" << std::endl;
   std::cout << "  weights2_data[0][0][0][0][0] = " << weights2_data[0][0][0][0][0] << std::endl;
   std::cout << "Layer 2, Site 3, phys_in=0, phys_out=0:" << std::endl;
   std::cout << "  weights2_data[3][0][0][0][0] = " << weights2_data[3][0][0][0][0] << std::endl;
   std::cout << "============================" << std::endl;
   ...
   for (const auto& sample : samples) {
      ...
      std::ifstream fref(inDir+"data/references/"+sample+".dat");
#ifdef RTL_SIM
      std::string RESULTS_LOG = inDir+"data/results_rtlsim/"+sample+".log";
#else
      std::string RESULTS_LOG = inDir+"data/results_csim/"+sample+quantization+".log";
#endif
      ...
      if (fin.is_open()) {
         ...
         while (std::getline(fin, iline)) {
            ...
            */
            ...
            tn4ad(encoded_input, weights1_data, weights2_data, &norm_score, &trigger_decision);
         ...
         data_t encoded_input[LAYER1_INPUT_SITES][LAYER1_PHYS_IN] = {{0}};
         data_t norm_score;
         bool trigger_decision;
