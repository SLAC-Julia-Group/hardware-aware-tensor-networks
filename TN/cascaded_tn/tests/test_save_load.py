...
class TestSaveLoad:
    ...
    def simple_model(self):
        ...
        return create_trainable_autoencoder(
            layer_dims=[10, 5, 3],
            bond_dims=[4, 3],
            phys_dims=[2, 2, 2],
    ...
    @pytest.fixture
    def symmetric_model(self):
        """Create a symmetric autoencoder model."""
        return create_trainable_autoencoder(
            layer_dims=[20, 10, 5],
            bond_dims=[6, 4, 4, 6],
            phys_dims=[2, 3, 2],
            enable_relu=[1],  # ReLU on second layer only
            cyclic=False,
            symmetric=True,
    ...
    def test_architecture_preservation(self, simple_model, temp_dir):
        ...
        assert original_config['symmetric'] == loaded_config['symmetric'], \
            f"Symmetric flag mismatch"
    ...
    def test_symmetric_model_save_load(self, symmetric_model, temp_dir):
        """Test save/load for symmetric autoencoder architectures."""
        filepath = os.path.join(temp_dir, 'test_symmetric.pkl')
        
        # Verify it's symmetric
        original_config = symmetric_model._extract_config()
        assert original_config['symmetric'] == True, "Model should be symmetric"
        
        # Save and load
        symmetric_model.save(filepath)
        loaded_model = CascadedModel.load(filepath)
        
        # Verify loaded model is also symmetric
        loaded_config = loaded_model._extract_config()
        assert loaded_config['symmetric'] == True, "Loaded model should be symmetric"
        
        # Verify architecture matches
        assert len(loaded_model.cascade.operators) == len(symmetric_model.cascade.operators), \
            "Number of operators mismatch in symmetric model"
